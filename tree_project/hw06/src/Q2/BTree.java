...
public class BTree<E extends Comparable<E>>  implements SearchTree<E> {
    ...
    private boolean insert(Node<E> root, E item) {
        ...
        if (index == -(root.size+1)  && item.compareTo(root.data[index]) == 0) {
            return false;
        }
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    /**
     * paramatre olarak verilen değeri b-tree'de arar ve eşleşme yakalarsa item'ın içinde olduğu node'u return eder.
     * @param item aranacak E ögesi
     * @return itemın icinde oldugu node
     */
    public Node<E> binarySearchInBtree(E item){
        return binarySearchInBtree(root,0,root.size,item);
    }

    /**
     * recursive olarak item'ı arayan method
     * @param Root item'ın icinde arancagı node
     * @param first ilk index
     * @param last aranacak son index
     * @param item aranacak öge
     * @return item'ın icinde oldugu node
     */
    private Node<E> binarySearchInBtree(Node<E> Root, int first, int last, E item){

        int index = binarySearchInNode(Root.data, first, Root.size, item);

        if (index >=0 ) {
            return Root;
        }

        if((item.compareTo(Root.data[0])) < 0){
            if(Root.child[0] != null)
                return binarySearchInBtree(Root.child[0],0,Root.child[0].size,item);
        }

        for(int i = 0; i< Root.size-1;i++){
            if(((item.compareTo(Root.data[i])) > 0) &&
               ((item.compareTo(Root.data[i+1])) < 0)){
                   if(Root.child[i+1] != null)
                       return binarySearchInBtree(Root.child[i+1],0,Root.child[i+1].size,item);
            }
        }

        if((item.compareTo(Root.data[Root.size-1])) > 0){
            if(Root.child[Root.size] != null)
    ...
    /**
     * Node içinde taget'i arayan method
     * @param arr node'un icindeki veriler array'i
     * @param first binarysearch'un baslayacagı ilk index
     * @param last search'un devam edecegi son index
     * @param target aranacak deger
     * @return target'ın arr icerindeki indec değeri, arama başarısızsa -(last + 1) degeri
     */
    private int binarySearchInNode(E[] arr, int first, int last, E target) {
        ...
        while(tempFirst <= tempLast){
            ...
            int mid = tempFirst + tempLast >>>1;
            E midData = arr[mid];
            int check = midData.compareTo(target);
    ...
    /**
     * Insert methodun kullanılan binarySearch methodu.
     * item arr içerisinde ise onun indeksini içerinde değil ise olması gereken indexi return eder.
     * @param item search edilecek eleman
     * @param arr binary search sırasında item'ın aranacagı array
     * @param first binary search sırasında item'a bakılacak ilk index
     * @param last binary search sırasında item'a bakılacak son index
     * @return index of item
     */
    private int binarySearch(E item,E[] arr, int first, int last){
        int result = binarySearchInNode(arr,first, last,item);
        if(result <0){
            if(item.compareTo(arr[0]) < 0){
                return 0;
            }
            for(int i = 0; i< (last-1);i++){
                if((item.compareTo(arr[i]) > 0) &&
                        (item.compareTo(arr[i+1]) < 0)){
                    return i+1;
                }
            }
            return last;
    ...
    public static void main(String[] args) {
        ...
        System.out.println(bTree.binarySearchInBtree(7));
